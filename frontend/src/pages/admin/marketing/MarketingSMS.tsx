...
import { sendSmsMessage, fetchMarketingHistory } from '../../../features/marketing/marketingSlice';
...
const MarketingSMS = () => {
  ...
  useEffect(() => {
    ...
    dispatch(fetchMarketingHistory('SMS'));
  }, [dispatch]);
  ...
  const handleSend = async () => {
    ...
    setLoading(true);
    await dispatch(sendSmsMessage({ userIds: selectedUsers, message }));
    await dispatch(fetchMarketingHistory('SMS'));
    setLoading(false);
    setMessage('');
    setSelectedUsers([]);
    alert("SMS campaign successfully sent! (Check backend logs for details)");
  };
    ...
    <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          ...
          <div>
            ...
            <div className="space-y-4">
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">Message Body (Max 160 chars)</label>
                <textarea 
                  value={message}
                  onChange={e => setMessage(e.target.value)}
                  maxLength={160}
                  placeholder="Limited time offer! Use code PROMO50 to get 50% off."
                  className="w-full h-48 bg-gray-50 border border-gray-200 rounded-lg p-4 focus:outline-none focus:ring-2 focus:ring-blue-500"
                />
                <div className="text-right text-xs text-gray-500 mt-1">{message.length}/160</div>
              </div>
              <button 
                onClick={handleSend}
              ...
              >
                ...
                <span>{loading ? 'Sending Campaign...' : 'Send SMS Campaign'}</span>
              </button>
            </div>
          </div>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
        <div className="px-6 py-4 border-b border-gray-200">
          <h2 className="text-lg font-semibold text-gray-900">SMS Campaign History</h2>
        </div>
