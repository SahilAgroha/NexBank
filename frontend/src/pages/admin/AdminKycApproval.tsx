...
import { fetchPendingKycUsers, fetchApprovedKycUsers, fetchUnverifiedUsers, approveKyc, rejectKyc, sendKycRequest, suspendKyc } from '../../features/kyc/kycSlice';
...
const AdminKycApproval = () => {
  ...
  useEffect(() => {
    if (activeTab === 'PENDING') {
      dispatch(fetchPendingKycUsers());
    } else if (activeTab === 'APPROVED') {
      dispatch(fetchApprovedKycUsers());
  ...
  const handleSelectUser = async (user: any) => {
    ...
    setLoadingDocs(true);
    try {
      const response = await api.get(`/admin/kyc/documents/${user.id}`);
  ...
  const handleApprove = () => {
    if (selectedUser) {
      dispatch(approveKyc(selectedUser.id));
      setSelectedUser(null);
      setUserDocuments([]);
    }
  };
  ...
  const handleReject = () => {
    if (selectedUser && rejectReason.trim() !== '') {
      dispatch(rejectKyc({ userId: selectedUser.id, reason: rejectReason }));
  ...
  const handleSendRequest = async (e: React.FormEvent) => {
    ...
    try {
      await dispatch(sendKycRequest({ userId: Number(requestUserId), dueDate: requestDueDate, message: requestMessage })).unwrap();
  ...
  const handleSuspendKyc = async (userId: number) => {
    ...
    try {
      ...
      await dispatch(suspendKyc({ userId, reason })).unwrap();
    ...
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">Review KYC, check approved history, and request compliance updates.</p>
        </div>
      </div>
